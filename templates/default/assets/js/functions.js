...
document.addEventListener('DOMContentLoaded', () => {
	...
	if (messageWrap) {
		...
		if (messageWrap.innerHTML.trim()) {
			...
			// подключим слушатели для двух событий, при которых будет вызываться метод, удаляющий сообщение с экрана (+Выпуск №149)
...
function addToCart() {
	...
	document.querySelectorAll('[data-addToCart]').forEach(item => {
		...
		item.addEventListener('click', e => {
			...
			// в св-во: id объекта: cart положим значение (здесь- id) атрибута: data-addToCart для текущего элемента 
			...
			if (cart.id && !isNaN(cart.id)) {
				...
				// в св-во: qty объекта: cart изначально положим null
				cart.qty = null
				...
				$.ajax({
					...
					success: res => {
						...
						try {
							...
							// +Выпуск №142 | Пользовательская часть | Добавление в корзину часть 4
